...
class TestTrinoEngineConnExecutor {
  ...
  def testExecuteLine: Unit = {
    ...
    val array = Array(engineconnConf, "wds.linkis.rm.instance=10"
      , engineconnConf, "label.userCreator=root-IDE"
      , engineconnConf, "ticketId=037ab855-0c41-4323-970d-7f75e71883b6"
      , engineconnConf, "label.engineType=trino"
      , engineconnConf, "wds.linkis.trino.url=https://trino.dev.com/hive/hivetest"
      , engineconnConf, "wds.linkis.trino.ssl.insecured=true"
      , engineconnConf, "wds.linkis.trino.user=root"
      , engineconnConf, "wds.linkis.trino.password=123456"
      , springConf, "eureka.client.serviceUrl.defaultZone=http://127.0.0.1:8761/eureka/"
      , springConf, "logging.config=classpath:log4j2.xml"
      , springConf, "spring.profiles.active=engineconn"
      , springConf, "server.port=35655"
      , springConf, "spring.application.name=linkis-cg-engineconn")
    this.init(array)
    ...
    val jdbcExecutor: TrinoEngineConnExecutor = engineFactory.newExecutor(1, engineCreationContext, engine).asInstanceOf[TrinoEngineConnExecutor]
